...
const ANNUAL_ACCOUNTS_BUCKET = "annual-accounts";

/**
 * Annual-account PDFs live in Supabase Storage so we can show the
 * original document to the user and skip re-downloading from NBB on
 * pipeline re-runs. The bucket is private; reads go through short-
 * lived signed URLs generated by the service-role client.
 *
 * Path convention: `{enterprise_number}/{reference}.pdf`. The
 * matching `storage_path` value is persisted on the filing_references
 * row by the pipeline.
 */
export class DocumentRepository {
  static create(): DocumentRepository | null {
    if (!hasSupabase()) return null;
    return new DocumentRepository();
  }

  /** Standard object path for a (CBE, filing reference) pair. */
  static pathFor(enterpriseNumber: string, reference: string): string {
    const safeRef = reference.replace(/[^A-Za-z0-9._-]/g, "_");
    return `${enterpriseNumber}/${safeRef}.pdf`;
  }

  /**
   * Upload a PDF to the annual-accounts bucket. Best-effort: returns
   * the path on success, null on failure. Errors are logged but never
   * thrown — the surrounding extraction must keep running even when
  ...
  async upload(
    ...
    const path = DocumentRepository.pathFor(enterpriseNumber, reference);
    try {
      const { error } = await client()
        .storage.from(ANNUAL_ACCOUNTS_BUCKET)
        .upload(path, bytes, {
          contentType: "application/pdf",
          upsert: true,
        });
      if (error) {
        log.warn("annual-account upload failed", {
          cbe: enterpriseNumber,
          reference,
          error: error.message,
        });
      ...
      log.warn("annual-account upload threw", {
        cbe: enterpriseNumber,
        reference,
        error: message,
      });
  ...
  /**
   * Generate a short-lived signed URL for a stored PDF. Returns null
   * if the path doesn't exist or the bucket call fails.
   */
  async signedUrl(
    path: string,
    ...
    try {
      ...
      if (error || !data) {
        log.warn("annual-account signedUrl failed", {
          path,
      ...
      const message = err instanceof Error ? err.message : String(err);
      log.warn("annual-account signedUrl threw", { path, error: message });
      return null;
    }
  }
}
